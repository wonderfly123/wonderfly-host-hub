...
exports.addTrackToQueue = async (req, res) => {
  try {
    ...
    // Check if Spotify is connected and device is set
    if (!event.spotify.isConnected || !event.spotify.preferredDeviceId) {
      return res.status(401).json({ message: 'Spotify not fully configured' });
    }
    ...
    await SpotifyService.addToQueue(
      accessToken, 
      trackUri, 
      event.spotify.preferredDeviceId
    );
